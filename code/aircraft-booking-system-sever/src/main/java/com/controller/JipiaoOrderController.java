...
public class JipiaoOrderController {
    ...
    @Autowired
    private AlipayService alipayService;//支付宝支付服务
...
//                            jipiaoOrderEntity.setCreateTime(date);//时间
    ...
    */
    ...
    public R alipay(@PathVariable("id") Long id, HttpServletRequest request) {
        logger.debug("alipay方法:,,Controller:{},,id:{}",this.getClass().getName(),id);
        try {
            // 获取订单信息
            JipiaoOrderEntity jipiaoOrder = jipiaoOrderService.selectById(id);
            if(jipiaoOrder == null) {
            ...
            JipiaoEntity jipiao = jipiaoService.selectById(jipiaoOrder.getJipiaoId());
            if(jipiao == null) {
                return R.error("航班信息不存在");
            }
            
            // 生成商户订单号
            String outTradeNo = "JIPIAO_" + id + "_" + System.currentTimeMillis();
            
            // 设置支付金额
            String totalAmount = jipiaoOrder.getJipiaoOrderTruePrice().toString();
            
            // 设置订单标题和描述
            String subject = "机票预订-" + jipiao.getJipiaoName();
            String body = "航班: " + jipiao.getJipiaoName() + ", 出发地: " + jipiao.getJipiaoChufadi() + ", 目的地: " + jipiao.getJipiaoMudidi();
            
            // 调用支付宝支付
            String payHtml = alipayService.pay(outTradeNo, totalAmount, subject, body);
            
            // 更新订单支付信息
            jipiaoOrder.setIsPay("待支付");
            jipiaoOrder.setOrderPayOrder(outTradeNo);
            jipiaoOrderService.updateById(jipiaoOrder);
            
            // 返回支付页面HTML
            return R.ok().put("data", payHtml);
        } catch (Exception e) {
            e.printStackTrace();
            return R.error("调用支付接口失败: " + e.getMessage());
        }
    }
     ...
     * 查询订单支付状态
     */
    @RequestMapping("/payStatus/{id}")
    public R payStatus(@PathVariable("id") Long id, HttpServletRequest request) {
        logger.debug("payStatus方法:,,Controller:{},,id:{}",this.getClass().getName(),id);
        try {
            // 获取订单信息
            JipiaoOrderEntity jipiaoOrder = jipiaoOrderService.selectById(id);
            if(jipiaoOrder == null) {
            ...
            // 返回订单支付状态
            return R.ok().put("isPay", jipiaoOrder.getIsPay());
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public R updatePayStatus(@RequestParam String orderNo, @RequestParam String tradeNo, HttpServletRequest request) {
        logger.debug("updatePayStatus方法:,,Controller:{},,orderNo:{},tradeNo:{}",this.getClass().getName(),orderNo,tradeNo);
        try {
            // 根据订单号查询订单
            EntityWrapper<JipiaoOrderEntity> ew = new EntityWrapper<JipiaoOrderEntity>();
            ew.eq("order_pay_order", orderNo);
            ...
            e.printStackTrace();
            return R.error("更新支付状态失败: " + e.getMessage());
        }
    }
